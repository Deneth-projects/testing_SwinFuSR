...
# -----------------------------------------------------------------------------------
# Self-contained, dependency-free replacements for timm.models.layers utilities
# (DropPath, to_2tuple, trunc_normal_). This removes the need to install `timm`
# just to run inference on CPU. Behaviour matches timm's implementation closely
# enough for both training and inference use-cases in this project.
# -----------------------------------------------------------------------------------
def _ntuple(n):
    def parse(x):
...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    """Simplified truncated normal initializer. Only used at model construction
    time; all values are overwritten when pretrained weights are loaded."""
    with torch.no_grad():
